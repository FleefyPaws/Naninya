...
module.exports.run = async (bot, message, args) => {
    ...
    const role = message.mentions.roles.first() ? message.mentions.roles.first() : message.guild.roles.fetch(role => role.id === args[0]);
    if (!role) {
        const no13permembed = new MessageEmbed()
            .setTitle("❌ Please mention a role or give the role id")
            .setColor('#FF0000')
        message.delete()
        return message.channel.send(no13permembed).then(msg => msg.delete({
            timeout: 5000
        }));
    }
    Mute.findOne({
        ...
        async (err, data) => {
            ...
            if (!data) {
                ...
                const no1permembed = new MessageEmbed()
                    ...
                    .setColor('#32CD32')
                newMuteRole.save().then(message.channel.send(no1permembed));
            } else {
                const no1permembed = new MessageEmbed()
                    .setTitle(`❌ The mute role is already set for this server!\nPlease do \`${bot.prefix}removemuterole\` to remove muterole!`)
                    .setColor('#FF0000')
                return message.channel.send(no1permembed).then(msg => msg.delete({
                    timeout: 5000
                }));
            }
        });
